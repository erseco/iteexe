...
def replaceLinks(matchobj, package_name):
    ...
    if do \
        ...
        return re.sub(
            r'(?i)href\s*=\s*"?([^>"]+)"?',
            r'''href="\1" onclick="browseURL('%s/%s/\1',this); return false"''' %
            (G.application.exeAppUri,
             clean_url),
            anchor)
    else:
        return anchor
...
class TextAreaElement(ElementWithResources):
    ...
    def process(self, request):
        ...
        if self.id in request.args:
            if not hasattr(
                ...
                hasattr(
                    self.field_idevice,
                    'type') and self.field_idevice.type != 'frameset'):
                # process any new images and other resources courtesy of
                # tinyMCE:
                ...
                self.field.content_wo_resourcePaths \
                    = self.field.MassageContentForRenderView(
                        self.field.content_w_resourcePaths)
                # and begin by choosing the content for preview mode, WITH
                # paths:
    ...
    def renderPreview(self, visible=True, class_="block"):
        ...
        if (hasattr(self.field.idevice, 'klass') and dT == 'HTML5'):
            if (self.field.idevice.klass ==
                    'readingIdevice' or self.field.idevice.klass == 'CasestudyIdevice'):
    ...
    def getMediaAdaptedStrippedHTML(self, htmlContent, mediaParams={}):
        ...
        if mediaConverter is not None:
            htmlContentMediaAdapted = mediaConverter.handleAudioVideoTags(
                htmlContent, mediaParams)
            mediaConverter.handleImageVersions(
                htmlContentMediaAdapted, mediaParams)

        for strToRemove in self.dontCountStrs:
            htmlContentMediaAdapted = htmlContentMediaAdapted.replace(
                strToRemove, "")

        htmlContentMediaAdapted = re.sub(
            re.compile(
                '<div(.)*?>',
                re.MULTILINE),
            '\1',
            htmlContentMediaAdapted)
    ...
    def getMediaAdaptedHTML(self):
        ...
        if mediaConverter is not None:
            htmlContentMediaAdapted = mediaConverter.handleAudioVideoTags(
                self.renderView())
            htmlContentMediaAdapted = mediaConverter.handleImageVersions(
                htmlContentMediaAdapted)
            return htmlContentMediaAdapted
        else:
            return self.renderView()
    ...
    def getXMLType(self):
        ...
        htmlContentMediaAdapted = self.getMediaAdaptedStrippedHTML(
            htmlContent, {"noresize": True})

        imgChars = self._countTagLength(htmlContentMediaAdapted, "img")
        audioChars = self._countTagLength(
            htmlContentMediaAdapted, "audio", True)
        videoChars = self._countTagLength(
            htmlContentMediaAdapted, "video", True)

        threshHold = 10

        "TODO: Make sure that we also look for the audio ..."
        remainingChars = len(htmlContentMediaAdapted) - \
            imgChars - audioChars - videoChars
    ...
    def renderXML(
        ...
        if ExportMediaConverter.autoMediaOnly or ExportMediaConverter.autoMediaOnly == "true":
            xmlType = self.MEDIA_ONLY_SLIDE

        if xmlType == self.MEDIA_ONLY_SLIDE:
            xml += "<%s type='mediaslide' id='%s'>\n" % (elementToMake, myId)
            htmlContentInc = self.getMediaAdaptedStrippedHTML(
                self.renderView(), {"resizemethod": "stretch"})
            scriptStart = htmlContentInc.find("<script")
            if scriptStart != -1:
                scriptEnd = htmlContentInc.find(
                    "</script>", scriptStart) + len("</script>")
                htmlContentInc = htmlContentInc[:scriptStart] + \
                    htmlContentInc[scriptEnd:]
    ...
    def renderView(
        ...
        return '<%s id="ta%s" class="%s iDevice_content"%s>%s%s%s</%s>%s' % (
            htmlTag, self.id, class_, visible, lb, content, lb, htmlTag, lb)
...
class FeedbackElement(ElementWithResources):
    ...
    def doRender(self, preview=False):
        ...
        if self.field.feedback != "":
            html += common.feedbackBlock(self.id,
                                         self.field.feedback,
...
class Feedback2Element(ElementWithResources):
    ...
    def renderEdit(self):
        ...
        if hasattr(self.field, 'buttonCaption'):
            html += '<p><label for="btnCaption' + self.id + \
                '">' + _("Button Caption") + ': </label>'
            html += common.textInput('btnCaption' +
                                     self.id, self.field.buttonCaption, 40)
            html += '</p>'
        html += common.formField('richTextArea', this_package,
                                 '', '',
                                 self.id, '',
...
class ImageElement(Element):
    ...
    def renderEdit(self):
        ...
        html += '<input type="button" onclick="%s(\'%s\')"' % (
            function, self.id)
...
class MultimediaElement(Element):
    ...
    def renderMP3(self, filename, mp3player):
        ...
        mp3Str_mat = common.flash(
            filename,
            self.field.width,
            self.field.height,
            id="mp3player",
            params={
                'movie': '%s?song_url=%s&song_title=%s' % (mp3player,
                                                           filename,
                                                           self.field.caption),
                'quality': 'high',
                'bgcolor': '#E6E6E6'})
...
class MagnifierElement(Element):
    ...
    def renderMagnifier(self, imageFile, magnifierFile):
        ...
        if magnifierFile == "mojomagnify.js":  # View
            ...
            html += '<a href="' + imageFile + '"><img src="' + imageFile + '" alt="" width="' + field.width + '" height="' + \
                field.height + '" class="magnifier-size-' + field.glassSize + ' magnifier-zoom-' + field.initialZSize + '" /></a>' + lb
            html += '</span>' + lb
        else:  # Preview
            html = '<img id="magnifier%s" src="%s" data-magnifysrc="%s"' % (
                self.id, imageFile, imageFile)
            if field.width != "":
                html += ' width="' + field.width + '"'
            if field.height != "":
                html += ' height="' + field.height + '"'
            html += ' data-size="%s"  data-zoom="%s" />' % (
                field.glassSize, field.initialZSize)
            html += lb
        return html
    ...
    def renderXML(self):
        ...
        for i, (text, missingWord) in enumerate(self.field.parts):
            if text:
                ...
                xml += ExportMediaConverter.removeHTMLTags(
                    text, ["table", "tbody", "tr", "td", "th"])
                breakCount += 1
            if missingWord:
    ...
    def renderView(self, feedbackId=None, preview=False):
        ...
        if preview:
            ...
            html += ['<form name="cloze-form-' + self.id +
                     '" action="#" class="activity-form cloze-activity-form">']
        ...
        for i, (text, missingWord) in enumerate(self.field.parts):
            ...
            if missingWord:
                ...
                if self.field.instantMarking:
                    inputHtml += [
                        '<input class="autocomplete-off" type="text" value="" id="clozeBlank%s.%s" style="width:%sch" onkeyup="$exe.cloze.change(this)" />' %
                        (self.id, i, lenWord)]
                else:
                    inputHtml += [
                        '<input class="autocomplete-off" type="text" value="" id="clozeBlank%s.%s" style="width:%sch" />' %
                        (self.id, i, lenWord)]
                html += inputHtml
        ...
        if self.field.instantMarking:
            html += [
                '<input type="button" value="%s" id="getScore%s" onclick="$exe.cloze.showScore(\'%s\')" />' %
                (c_("Get score"), self.id, self.id)]
            ...
            if feedbackId is not None:
                html += [
                    common.feedbackButton(
                        ...
                        self.id,
                        c_("Show Feedback"),
                        onclick="$exe.cloze.toggleFeedback('%s',this)" %
                        self.id)]
            ...
            if preview:
                html += [common.button('submit%s' %
                                       self.id, c_("Submit"), id='submit%s' %
                                       self.id, onclick="$exe.cloze.submit('%s')" %
                                       self.id)]
            else:
                html += [common.submitButton('submit%s' %
                                             self.id, c_("Submit"), id='submit%s' %
                                             self.id)]
            html += [
                common.button(
        ...
        html += [' ', common.button('%sshowAnswersButton' %
                                    self.id, value, id='showAnswersButton%s' %
                                    self.id, style=style, onclick=onclick)]
    ...
    def renderView(self, feedbackId=None, preview=False):
        ...
        for i, (text, missingWord) in enumerate(self.field.parts):
            ...
            if missingWord:
                ...
                if self.field.instantMarking:
                    inputHtml.insert(
                        2, '  onkeyup="$exe.cloze.onLangChange(this)"')
                html += inputHtml
                # Hidden span with correct answer
                html += ['<span style="display:none" ',
                         'id="clozelangAnswer%s.%s">%s</span>' % (self.id,
                                                                  i,
                                                                  encrypt(missingWord))]
        ...
        if self.field.instantMarking:
            html += [
                '<input type="button" value="%s" id="getScore%s" onclick="$exe.cloze.showLangScore(\'%s\')" />' %
                (c_("Get score"), self.id, self.id)]
            ...
            if feedbackId is not None:
                html += [
                    common.feedbackButton(
                        'feedback' +
                        self.id,
                        c_("Show/Hide Feedback"),
                        onclick="$exe.cloze.toggleLangFeedback('%s')" %
                        self.id)]
            # Set the show/hide answers button attributes
            ...
            html += [
                common.button(
                    'submit%s' %
                    self.id, c_("Submit"), id='submit%s' %
                    self.id, onclick="$exe.cloze.langSubmit('%s')" %
                    self.id), common.button(
                    'restart%s' %
                    self.id, c_("Restart"), id='restart%s' %
                    self.id, style="display:none", onclick="$exe.cloze.langRestart('%s')" %
                    self.id)]
            # Set the show/hide answers button attributes
            style = 'display:none'
            value = c_("Show Answers")
...
class MathElement(Element):
    ...
    def renderEdit(self):
        ...
        html += common.insertSymbol("input" + self.id,
                                    ...
                                    _("text"),
                                    14)
        html += common.insertSymbol("input" +
                                    self.id, "", "", r"\\\\\n", _("newline"))
        # font size select
        html += '<br/>'
...
class SelectOptionElement(Element):
    ...
    def renderEdit(self):
        ...
        """
        html = "<tr><td align=\"left\"><b id='ans%s-editor-label'>%s</b>" % (
            self.id, _("Option"))
        ...
        if self.index == 0:
            html += common.elementInstruc(
                self.field.question.correctAnswerInstruc)
    ...
    def renderView(self, preview=False):
        ...
        html += '<label for="op' + ident + '" class="sr-av"><a href="#answer-' + \
            self.id + '">' + c_("Option") + ' ' + str(self.index + 1) + '</a></label>'
        ...
        # Answer feedback
        html += '<' + sectionTag + ' class="iDevice_answer-feedback feedback" id="feedback-' + \
            ident + '" style="display:none"></' + sectionTag + '>' + lb
...
class SelectquestionElement(Element):
    ...
    def process(self, request):
        ...
        if "action" in request.args and \
           request.args["action"][0] == "del" + self.id:
            # before deleting the questions object, remove any internal
            # anchors:
    ...
    def renderEdit(self):
        ...
        html += "<b id='question" + self.id + \
            "-editor-label'>" + _("Question:") + " </b>"
    ...
    def doRender(self, img, preview=False):
        ...
        if preview:
            ...
            html += '<' + titleTag1 + ' class="js-sr-av">' + \
                c_("Question") + '</' + titleTag1 + '>' + lb
            html += self.questionElement.renderPreview()
        else:
            html += '<form name="multi-select-form-' + self.id + \
                '" action="#" class="activity-form multi-select-form">' + lb
            html += '<' + titleTag1 + ' class="js-sr-av">' + \
                c_("Question") + '</' + titleTag1 + '>' + lb
        ...
        html += '<' + titleTag2 + ' class="js-sr-av">' + \
            c_("Answers") + '</' + titleTag2 + '>' + lb
        for element in self.options:
        ...
        html += '<p>' + lb
        html += '<input type="button" name="submitSelect" class="feedbackbutton multi-select-feedback-toggler" value="%s" id="multi-select-feedback-toggler-%d-%s" /> ' % (
            c_("Show Feedback"), len(self.field.options), self.field.id)
        html += lb
        html += '</p>' + lb
        html += '</div>' + lb
        ...
        # Feedback
        html += '<' + sectionTag + \
            ' id="%s" class="js-hidden js-feedback">' % ("f" + self.field.id)
        html += lb
        html += '<' + titleTag2 + ' class="js-sr-av">' + \
            c_("Feedback") + '</' + titleTag2 + '>' + lb
        html += lb
        if preview:
...
class QuizOptionElement(Element):
    ...
    def renderXML(self):
        ...
        # convert any audio tags etc. that have been found for the current
        # profile
        feedbackMediaAdapted = ExportMediaConverter.getInstance(
        ).handleAudioVideoTags(self.feedbackElement.renderView())
        audioStr = ExportMediaConverter.getInstance().makeAudioAttrs(feedbackMediaAdapted)
        xml += "<feedback %s>\n<![CDATA[\n" % audioStr
        xml += ExportMediaConverter.trimHTMLWhiteSpace(
    ...
    def renderAnswerView(self, preview=False):
        ...
        html += '<p class="iDevice_answer-field js-required">' + lb
        html += '<label for="i' + self.id + '" class="sr-av"><a href="#answer-' + \
            self.id + '">' + c_("Option") + ' ' + str(self.index + 1) + '</a></label>'
        html += '<input type="radio" name="option%s" ' % self.field.question.id
        html += 'id="i%s" ' % self.id
        html += 'class="exe-radio-option exe-radio-option-%d-%d-%s-multi"/>' % (
    ...
    def renderFeedbackView(self, preview=False):
        ...
        feedbackStr = ""
        if hasattr(
                self.feedbackElement.field,
                'content') and self.feedbackElement.field.content.strip() != "":
            if preview:
        ...
        html = '<' + sectionTag + \
            ' id="sa%sb%s" class="feedback js-hidden">' % (str(self.index), self.field.question.id)
        if dT != "HTML5":
            html += '<a name="sa%sb%s"></a>' % (
    ...
    def renderNoscript(self, preview):
        ...
        if self.field.isCorrect:
            ...
            html += c_("Wrong")
        html += '</a><span> (<a href="#sa' + str(self.index) + 'b' + \
            self.field.question.id + '">' + c_("Feedback") + '</a>)</span></li>'
        html += lb
        '''
        if preview:
...
class QuizQuestionElement(Element):
    ...
    def renderXML(self):
        ...
        ).handleAudioVideoTags(questionStr)

        # this causes a formatting problem with LWUIT HTMLComponent for some
        # reason
        questionMediaAdjusted = questionMediaAdjusted.replace(
            "align=\"right\"", "")
    ...
    def doRender(self, img1, img2, preview=False):
        ...
        if preview:
            ...
            html = '<form name="multi-choice-form-' + self.id + \
                '" action="#" onsubmit="return false" class="activity-form">' + lb
            html += '<' + titleTag1 + ' class="js-sr-av">' + \
                c_("Question") + '</' + titleTag1 + '>' + lb
            html += self.questionElement.renderView()
        ...
        if self.hintElement.field.content.strip():
            if preview:
                html += common.ideviceHint(
                    self.hintElement.renderPreview(), "preview", "h4")
            else:
                html += common.ideviceHint(
                    self.hintElement.renderView(), "view", "h4")
