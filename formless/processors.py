    ...
    def process(self, context, boundTo, data):
        ...
        def _finish(ignored):
            if not failures:
                ...
                # print "There were validation errors. The errors were: ",
                # failures
        ...
        for binding in typedValue.arguments:
            ...
            try:
                ...
                results[name] = iformless.IInputProcessor(
                    binding.typedValue).process(
                    context, boundTo, data.get(
                        name, ['']))
...
class ProcessPropertyBinding(compy.Adapter):
    ...
    def process(self, context, boundTo, data, autoConfigure=True):
        ...
        result = {}
        try:
            result[binding.name] = iformless.IInputProcessor(binding.typedValue).process(
                context, boundTo, data.get(binding.name, ['']))
        except formless.InputError as e:
            result[binding.name] = data.get(binding.name, [''])
            raise formless.ValidateError(
                {binding.name: e.reason}, e.reason, result)
...
def process(typed, data, configurable=None, ctx=None):
    ...
    try:
        return iformless.IInputProcessor(typed).process(
            ctx, configurable, data, autoConfigure=False)
    except TypeError:
        return iformless.IInputProcessor(
            typed).process(ctx, configurable, data)
