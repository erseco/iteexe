...
class BaseInputRenderer(compy.Adapter):
    ...
    def rend(self, context, data):
        ...
        if data.typedValue.getAttribute('immutable'):
            ...
            # TODO: If flattening this results in an empty string, return an
            # empty string
            inp = invisible(
                render=lambda c,
                value: self.input(
...
class StringRenderer(BaseInputRenderer):
    def input(self, context, slot, data, name, value):
        if data.typedValue.getAttribute('hidden'):
            ...
            T = "text"
        return slot[eval(input(id=keyToXMLID(context.key),
                               type=T,
                               name=name,
                               value=value,
    ...
    def input(self, context, slot, data, name, value):
        ...
        node = eval(
            input(
                ...
                name=name,
                value='True',
                _class="freeform-input-checkbox"))
        if value:
...
class FileUploadRenderer(BaseInputRenderer):
    def input(self, context, slot, data, name, value):
        return slot[eval(input(id=keyToXMLID(context.key),
                         type="file", name=name, _class='freeform-input-file'))]
...
class ChoiceRenderer(BaseInputRenderer):
    default_select = select(
        ...
        option(
            ...
            render=isSelected)[
                lambda c,
            d: iformless.ITyped(c).stringify(d)]]
...
class RadioRenderer(ChoiceRenderer):
    default_select = span(id=slot('id'),
                          render=directive('sequence'))[div(pattern="item",
                                                            _class="freeform-radio-option")[input(type="radio",
                                                                                                  name=slot('name'),
                                                                                                  value=valToKey,
                                                                                                  render=isSelected)[lambda c,
                                                                                                                     d: iformless.ITyped(c).stringify(d)]]]
...
class GroupBindingRenderer(compy.Adapter):
    ...
    def rend(self, context, data):
        ...
        configurable = conf.GroupConfigurable(
            data.boundTo, data.typedValue.interface)
        ...
        def generateBindings():
            for name in bindingNames:
                ...
                renderer = iformless.IBindingRenderer(
                    bnd, defaultBindingRenderer, persist=False)
...
class PropertyBindingRenderer(BaseBindingRenderer):
    def rend(self, context, data):
        context.remember(data, iformless.IBinding)
        context.remember(data.typedValue, iformless.ITyped)
        typedRenderer = iformless.ITypedRenderer(
            data.typedValue, defaultBindingRenderer, persist=False)
...
class MethodBindingRenderer(BaseBindingRenderer):
    def rend(self, context, data):
        ...
        if not self.isGrouped:
            ...
            button_pattern.fillSlots(
                'input',
                eval(
                    input(
                        type='submit',
                        value=data.action or data.label)))
    ...
    def generateArguments(self, context, args):
        ...
        for argument in args:
            try:
                content_pattern = context.tag.patternGenerator(
