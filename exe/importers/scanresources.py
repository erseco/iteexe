...
def relpath(path, start):
    ...
    if sys.platform[:3] == 'win':
        ...
        if os.path.splitdrive(start)[1] in [
                '', os.curdir, os.sep, os.sep + os.curdir]:
...
class Resources:
        ...
        for root, dirs, files in self._safewalk(self.baseurl):
            ...
            if self.client:
                self.client.call(
                    ...
                    _('Analizing directory %d of %d: %s') %
                    (i,
                     self.numdirs,
                     root.encode(
        ...
        for url in list(self.resources['urls'].values()):
            if url.type == 'dir':
                if self.client:
                    self.client.call(
                        'eXe.app.getController("Toolbar").updateImportProgressWindow',
                        _('Calculating relative paths to directory %d of %d: %s') %
                        (i,
                         self.numdirs,
                         url.relpath.encode(
                             sys.getfilesystemencoding())))
    ...
    def _computeLinks(self):
        ...
        for url in htmls:
            if self.cancel:
                return
            if self.client:
                self.client.call('eXe.app.getController("Toolbar").updateImportProgressWindow', _(
                    'Analyzing HTML file labels %d of %d: %s') % (i, total, str(url)))
            ...
            if declaredHTMLEncoding:
                content = UnicodeDammit(
                    content, [declaredHTMLEncoding]).unicode_markup
                ...
                for key, value in tag.attrs.items():
                    ...
                    for val in value:
                        ...
                        for l, rl in self.resources['urls'][url.parentpath].relpaths:
                            ...
                            if rl in unq_value:
                                L = Link(
                                    self.resources['urls'][l], rl, url, tag, key, rl)
                                matches.append(L)
                            elif low_rl in unq_value:
                                L = Link(
                                    self.resources['urls'][l], rl, url, tag, key, low_rl)
                                matches.append(L)
                            elif l in unq_value:
                                L = Link(
                                    self.resources['urls'][l], rl, url, tag, key, l)
                                matches.append(L)
                matches_final = []
                for l1 in matches:
            ...
            i += 1
        csss = self.resources['mimes']['text/css'] if 'text/css' in list(
            self.resources['mimes'].keys()) else None
        ...
        for url in csss_and_htmls:
            ...
            if self.client:
                self.client.call('eXe.app.getController("Toolbar").updateImportProgressWindow', _(
                    'Exhaustively analyzed file %s %d of %d: %s') % (tipo, i, total, str(url)))
            matches = []
            for l, rl in self.resources['urls'][url.parentpath].relpaths:
                low_rl = rl.lower()
                if rl in content:
            ...
            if matches_final:
                for match in matches_final:
                    if not [
                            link for link in url.links if link.relative == match.relative]:
                        url.addLink(match)
                        url.addRLink(str(match.url))
            i += 1
    ...
    def _guessName(self, url):
        ...
        for link in url.plinks:
            if link.tag.contents and isinstance(
                    link.tag.contents[0],
                    str) and link.tag.contents[0].lstrip() != "":
    ...
    def _insertNode(self, node, url, depth=0, idevice=None):
        ...
        if url.type == "file":
            ...
            if link and link.relative not in link.referrer.contentUpdated:
                if link.match:
                    ...
                    link.referrer.content = link.referrer.content.replace(
                        link.relative, '###resources###/%s' % (url.storageName))
        ...
        content = url.getContent()
        if content:
            content_w_resourcePaths = re.sub(
            ...
            if url.mime == "text/html" and idevice:
                ...
                if soup and soup.declaredHTMLEncoding:
                    content_w_resourcePaths = re.sub(
                        soup.declaredHTMLEncoding, 'utf-8', content_w_resourcePaths, re.IGNORECASE)
                    content_wo_resourcePaths = re.sub(
                        soup.declaredHTMLEncoding, 'utf-8', content_wo_resourcePaths, re.IGNORECASE)
