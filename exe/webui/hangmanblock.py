...
class HangmanBlockInc(Block):
    ...
    def __init__(self, parent, idevice):
        ...
        # go through all image fields in the list and create an image element
    ...
    def process(self, request):
        ...
        if blankWords is True:
            self.idevice.message = _(
                "One or more words or hints are blank.  Please do not have any blank hints or words - you can delete unused ones.")
        ...
        for wordElement in self.wordElements:
            ...
            if "action" in request.args and request.args["action"][0] == wordElement.id:
                ...
                self.idevice.hintTextFields.remove(
                    self.hintElements[wordIdx].field)
    ...
    def _renderGame(self, style, mode="view"):
        ...
        html += "<div id='hangman%(gameId)smessageStore' style='display: none'>" % {
            "gameId": hangmanGameId}
        html += self._renderHTMLElement(mode,
                                        self.wrongGuessTextElement,
                                        "hmwrong" + hangmanGameId)
        html += self._renderHTMLElement(mode,
                                        self.lostLevelTextElement,
                                        "hmlost" + hangmanGameId)
        html += self._renderHTMLElement(mode,
                                        self.levelPassedTextElement,
                                        "hmpassed" + hangmanGameId)
        html += self._renderHTMLElement(mode,
                                        self.gameWonTextElement,
                                        "hmwon" + hangmanGameId)
        ...
        for imgIndex, imgElement in enumerate(self.chanceImageElements):
            html += "hangman_chanceimgids['%(gameId)s'][%(index)d] = '%(imgdivid)s';\n" % {
                "index": imgIndex,
                "imgdivid": "hangman" + self.id + "img" + imgElement.id,
                "gameId": hangmanGameId}
        ...
        for imgElement in self.chanceImageElements:
            if imgElement.field.imageResource and imgElement.field.imageResource is not None:
                html += "<div id='hangman" + self.id + "img" + \
                    imgElement.id + "' style='display: none'>"
...
<div id="%(gameId)s_gamearea" style='width: %(width)dpx;' class='exehangman_gamearea'>
        ...
        """ % {
            ...
            'hintStyle': self.hintFieldStyleElement.renderView(),
            'wordStyle': self.wordAreaStyleElement.renderView(),
            'resetText': self.resetButtonTextElement.renderView(),
    ...
    def renderEdit(self, style):
        ...
        for imgElement in self.chanceImageElements:
            ...
            html += common.submitImage(imgElement.id,
                                       imgElement.field.idevice.id,
                                       "/images/stock-cancel.png",
                                       _("Remove This Life")) + "<br/>"

        addChanceButtonLabel = _("Add Chance")
        html += common.submitButton("addChance" +
                                    str(self.id), addChanceButtonLabel)
        html += "<br/>"
