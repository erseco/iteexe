...
class lomSub(supermod.lom):
            ...
            if base.endswith('_dateTime'):
                ...
                if v.startswith('BEGIN:VCARD'):
                    if not v.endswith(' END:VCARD'):
                        v = v + ' END:VCARD'
                    if re.findall(
                        '^BEGIN\\:VCARD VERSION\\:3\\.0 FN\\:.*\\ EMAIL\\;TYPE\\=INTERNET\\:.*\\ ORG\\:.*END\\:VCARD$',
                            v):
                        sep = []
                ...
                v = rootObj.get_valueOf_()
                d = re.findall(
                    '^P([0-9]+Y){0,1}([0-9]+M){0,1}([0-9]+D){0,1}(T([0-9]+H){0,1}([0-9]+M){0,1}([0-9]+(\\.[0-9]+){0,1}S){0,1}){0,1}$',
                    v)
                if d:
                ...
                try:
                    form[base] = self.sourceMap[rootObj.get_valueOf_().encode(
                        'utf-8')] + '_%s' % rootObj.language
    ...
    def getindex(self, field):
        ...
        if num:
            num = num[0]
            name = field[: -len(num)]
            if name not in list(self.__index__.keys()):
                self.__index__[name] = {}
            if num not in self.__index__[name]:
                self.__index__[name][num] = len(
                    list(self.__index__[name].keys()))
...
class copyrightAndOtherRestrictionsVocabSub(
        supermod.copyrightAndOtherRestrictionsVocab):
    def __init__(self, source=None, value=None, extensiontype_=None):
        super(
            copyrightAndOtherRestrictionsVocabSub,
            self).__init__(
            source,
            value,
            extensiontype_,
...
class copyrightAndOtherRestrictionsSub(supermod.copyrightAndOtherRestrictions):
    def __init__(
            ...
            uniqueElementName=None,
            extensiontype_=None):
        super(copyrightAndOtherRestrictionsSub, self).__init__(
            source, value, uniqueElementName, extensiontype_, )
...
class copyrightAndOtherRestrictionsValueSub(
        supermod.copyrightAndOtherRestrictionsValue):
    def __init__(
            self,
            source=None,
            value=None,
            ...
            valueOf_=None):
        super(
            copyrightAndOtherRestrictionsValueSub,
            self).__init__(
            source,
            value,
            uniqueElementName,
