...
class TestoptionElement(object):
    ...
    def renderView(self, preview=False):
        ...
        html += '<label for="' + fieldId + '" class="sr-av"><a href="#answer-' + \
            fieldId + '">' + c_("Option") + ' ' + str((self.index + 1)) + '</a></label>'
        html += '<input type="radio" name="' + self.keyId + \
            '" id="' + fieldId + '" value="' + str(self.index) + '" />'
        html += lb
        html += '</p>' + lb
