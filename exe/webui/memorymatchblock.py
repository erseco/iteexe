...
class MemoryMatchBlockInc(Block):
    ...
    def process(self, request):
        ...
        try:
            ...
            # only here in case there is an invalid integer, in which case
    ...
    def renderXML(self, style):
        ...
        xml += self.idevice.mainFieldSet.applyFileTemplateToDict(
            mainDict, "memmatch_template_head.xml", previewMode)
        ...
        if mediaConverter is not None:
            ...
            mainDict['feedbackpositive'] = ExportMediaConverter.trimHTMLWhiteSpace(
                mainDict['feedbackpositive'])

            mainDict['feedbacknegative'] = mediaConverter.reprocessHTML(
                mainDict['feedbacknegative'])
            mainDict['feedbacknegative'] = ExportMediaConverter.trimHTMLWhiteSpace(
                mainDict['feedbackpositive'])

            mediaConverter.resizeImg(
                XMLPage.currentOutputDir /
                mainDict['cellbackImg_imgsrc'],
                50,
                50,
                {},
                {
        ...
        xml += self.idevice.mainFieldSet.applyFileTemplateToDict(
            mainDict, "memmatch_template_footer.xml", previewMode)
    ...
    def _renderGame(self, style, previewMode):
        ...
        firstFrag = self.idevice.mainFieldSet.applyFileTemplateToDict(
            mainDict, "memmatch_template_head.html", previewMode)
        html += firstFrag
        for memoryMatchElement in self.memoryMatchElements:
            thisObjDict = memoryMatchElement.field.mainFields.getRenderDictionary(
                memoryMatchElement.elements, "obj", previewMode)
        ...
        footerFrag = self.idevice.mainFieldSet.applyFileTemplateToDict(
            mainDict, "memmatch_template_footer.html", previewMode)
        html += footerFrag
    ...
    def renderEdit(self, style):
        ...
        html += common.submitButton("calcPairs" +
                                    str(self.id), _("Create / Trim Pairs"))
        html += "<br/>"
