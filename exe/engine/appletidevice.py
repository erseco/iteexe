...
# Constants
GEOGEBRA_FILE_NAMES = set(["geogebra.jar",
                           "geogebra_algos.jar",
                           "geogebra_cas.jar",
                           "geogebra_export.jar",
                           "geogebra_gui.jar",
...
DESCARTES_FILE_NAMES = set(["Descartes.jar",
                            "Descartes3.jar",
                            "Descartes4.jar",
                            "Descartes4Runtime.jar",
                            "DescartesA.jar",
                            "Descartes_A.jar",
                            "DescartesCalc.jar",
                            "Descartes_R.jar",
...
class AppletIdevice(Idevice):
    ...
    def __init__(self, parentNode=None):
        ...
        self.appletCode = ""
        self.type = "other"
        self._fileInstruc = x_("""Add all the files provided for the applet
...
files, once loaded will be displayed beneath the Applet code field.""")
        self._codeInstruc = x_(""""Find the .txt file (in the applet file)
and open it. Copy the contents of this file <ctrl A, ctrl C> into the applet
code field.""")
        self._typeInstruc = x_(""" <p>If the applet you're adding was generated
...
you created in Geogebra.</p>""")
    ...
    def uploadFile(self, filePath):
        ...
        if self.type == "descartes" and (
            ...
            log.debug("uploadFile " + str(filePath))
    ...
    def downloadFiles(self, stringapplet):
        ...
        for im in imageslist:
            ...
            stringappletmod = stringappletmod.replace(
                im, im[im.rfind("/") + 1:])
        ...
        for pathimg in urlimageslist:
            # we trick to urlimageslist adding files that haven't been detected
            # really
        ...
        for pathimgext in urlimgslistextended:
            ...
            try:
                resp = urllib.request.urlopen(pathimgext)
            ...
            try:
                ...
                img_down = urllib.request.urlretrieve(pathimgext, img)
            except BaseException:
                    ...
                    if result:
                        ...
                        for y in key_macro:
                            if string.find(tag, y) != -1:
                                ...
                                # and sometimes they are locals and sometimes
                                # they are inside the jar file
                                # notice unicode conversion
                                wexpression = r"%s'(?P<bla>[\w./]+)'" % y
        ...
        for pathmacro in urlmacroslist:
            ...
            try:
                resp = urllib.request.urlopen(pathmacro)
            except urllib.error.URLError as e:
                if not hasattr(e, "code"):
                    raise
                resp = e
            if resp.code == 200:
                macro_down = urllib.request.urlretrieve(pathmacro, macro)
                self.uploadFile(macro_down[0])
                os.remove(macro_down[0])

                # and modify the applet code inside eXe, now the file will be
                # local:
                for x in macroslist:
                    if x.endswith('.txt'):
            ...
            if filename.endswith(".html") or filename.endswith(".htm"):
                ...
                if filename.find(",") == -1:
                    ...
                    if self.verifyConn(filename) == False:
                        assert self.parentNode.package, _(
                            'Sorry, this URL is unreachable')
                        return
                    # filename is reachable, go on:
                    htmlbytes = urllib.request.urlopen(filename)
                else:
                    if self.verifyConn(filename[2:]) == False:
                        return html == ''
                    htmlbytes = urllib.request.urlopen(filename[2:])
                content = htmlbytes.read()
                # content = content.replace('""','"') Galo swears it won't be
                # necessary
                soup = BeautifulSoup(content)
                i = 0
                appletslist = []
                for ap_old in soup.find_all(
                ...
                for ap_supernew in soup.find_all(
                    "applet", {
                        "code": "descinst.DescartesWeb2_0.class"}):
                    DESC_PLUGIN = 1
                    for resource in reversed(self.userResources):
                        if resource._storageName != 'descinst.jar':
                            resource.delete()
    ...
    def copyFiles(self):
        ...
        if self.type == "descartes":
            ...
            self.message = ""
            self._typeInstruc = x_(
                """Please enter: scene number,URL (no spaces) that include it, eg: 3,http://example.com; click on the Upload button afterwards.""")
        if self.type == "other":
            self._typeInstruc = x_(
                """Click on the AddFiles button to select the .html or .htm file and then click on the Upload button to get its content.""")
