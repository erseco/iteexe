...
class _Resource(Persistable):
            ...
            if package is None:
                ...
                if hasattr(self._package, 'resourceDir'):
                    ...
                    log.warn(
                        ...
                        repr(package) +
                        "; setting to None.")
                # either way, play it safe and set its package to None and
                # bail:
                self._package = None
            return
    ...
    def uniquePath(self, original, unique):
        if original.exists() and original.isfile():
            unique += 1
            # Get the position of the last '.' which is equivalent of the
            # extension
    ...
    def _fn2ascii(self, filename):
        ...
        # JR: Convertimos el nombre del fichero a algunos caracteres ascii
        # validos
        import unicodedata
        import string
        # if the variable is actived and package is set and change, format name
        # of images and extension
        if G.application.config.cutFileName == "1" and self._package is not None:
            if self._package.isChanged:
                nameBase = nameBase[0:8]
                ext = self.cutExtension(ext, nameBase)
        validFilenameChars = "~,()[]-_. %s%s" % (string.ascii_letters, string.digits)
        if isinstance(nameBase, str):
            nameBase = str(nameBase)
        if isinstance(ext, str):
            ext = str(ext)
        cleanedBasename = unicodedata.normalize(
            'NFKD', nameBase).encode(
            'ASCII', 'ignore')
        nameBase = ''.join(
            c for c in cleanedBasename if c in validFilenameChars)
        cleanedExt = unicodedata.normalize(
            'NFKD', ext).encode(
            'ASCII', 'ignore')
    ...
    def cutExtension(self, ext, nameBase):
        ...
        if len(ext) > 4:
            if ext == '.jpeg':
                ...
                self.warningMsg = (
                    _('Unknown extension %s of file %s%s can\'t be transformed to ISO 9660.') %
                    (ext, nameBase, ext))
                return ext
        else:
            return ext
...
class Resource(_Resource):
    ...
    def __init__(self, owner, resourceFile):
        ...
        """
        self.checksum = Path(
            resourceFile).md5  # Just use the path name as a unique ID
            ...
            if hasattr(self, '_idevice') and self._idevice is not None:
                if hasattr(self._idevice, 'parentNode') \
                    ...
                    if hasattr(self._idevice.parentNode, 'package')\
                        ...
                        if hasattr(
                                self._idevice.parentNode.package,
                                'resourceDir') and self._idevice.parentNode.package.resourceDir is not None:
                            this_resource_path = \
    ...
    def addSelfToPackageList(self):
        ...
        if self._package:
            if not hasattr(self._package, 'resourceDir'):
                ...
                log.warn(
                    "resource " +
                    repr(self) +
                    " in addSelfToPackage with " +
                    "invalid self._package = \"" +
                    self._package._name +
                    "\" " +
                    repr(
                        self._package) +
        ...
        if self.path.isfile():
            ...
            log.error(
                'Resource file "%s" not found. Deleting resource object' %
                self.path)
            if self._idevice:
                self._idevice.userResources.remove(self)
    ...
    def __repr__(self):
        ...
        """
        return '<%s.%s for "%s" at %s>' % (
            __name__, self.__class__.__name__, self._storageName, id(self))
        ...
        if found_new_name:
            ...
            def updateIdevice(self):
                ...
                if isinstance(self._idevice, AppletIdevice):
                    ...
                    raise Exception(
                        _("renamed a Resource for an Applet Idevice, and it should not have even made it this far!"))
                    ...
                    # be renamed with the merge) might not have been updated
                    # yet!
                    ...
                    if this_field is not None\
                        ...
                        log.warn(
                            "Unaware of any other specific resource-renaming " +
    ...
    def testForZombieResources(self, deleteZombie=False):
        ...
        if self._package is not None\
            ...
            if not deleteZombie:
                ...
                log.warn(
                    "2nd pass: Checking zombie Resource \"" +
                    str(self) +
        ...
        if self._package is None \
            ...
            if found_idevice is None:
                if deleteZombie:
                    ...
                    log.warn(
                        "1st pass: not yet removing zombie Resource \"" +
                        str(self) +
                        "\"; no corresponding iDevice found.")
                    # but add the second-pass call,
                    # after cycling through all other resources 1 time:
