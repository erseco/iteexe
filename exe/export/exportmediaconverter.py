...
class ExportMediaConverter(object):
    ...
    def setCurrentPackage(self, pkg):
        if self.configParser is None:
            print(
                "setCurrentPackage says - Eh - you need to load the config values before you call me buddy")

        self.currentPackage = pkg
        if self.currentPackage.mxmlheight != "":
            self.configParser.set(
                "media", "maxheight", self.currentPackage.mxmlheight)
        if self.currentPackage.mxmlwidth != "":
            self.configParser.set(
                "media", "maxwidth", self.currentPackage.mxmlwidth)
    ...
    def handleImageVersions(self, htmlContent, mediaParams={}):
        ...
        startIndex = self._findNextTagStart(
            htmlContentLower, startIndex, ['img'])
        ...
        while startIndex != -1:
            ...
            currentWidth = self._getTagAttribVal(
                htmlContent, "width", startIndex)
            currentHeight = self._getTagAttribVal(
                htmlContent, "height", startIndex)
            ...
            if not ("noresize" in mediaParams and mediaParams["noresize"]):
                if imgPath in self.resizedImages:
                    resizeResult = self.resizedImages[imgPath]
                else:
                    resizeResult = self.resizeImg(
                        imgPath, -1, -1, resizeParams, mediaParams)

            if resizeResult is not None:
                widthAttribInfo = {}
                heightAttribInfo = {}

                widthAttr = self._getTagAttribVal(
                    htmlContent, "width", startIndex, widthAttribInfo)
                ...
                heightAttr = self._getTagAttribVal(
                    htmlContent, "height", startIndex, heightAttribInfo)
            ...
            if longDesc != tagContent:
                # We have found something - try and explode it into key/value
                # pairs
                ...
                for currentPart in longDescParts:
                    ...
                    # find out for which profile this is (e.g. portrait,
                    # landscape, or square)
                    ...
                    if profileName == screenProfile:
                        ...
                        if propertyName == "altsrc":
                            ...
                            for i in range(len(clipVals)):
                                clipParams[clipPropertyNames[i]] = int(
                                    clipVals[i])
                    ...
                    if altSrc is not None:
                        ...
                        htmlContent = htmlContent[:startIndex] + \
                            replacedTagContent + htmlContent[endOfTagIndex + 1]
                        endOfTagIndex = startIndex + len(replacedTagContent)

            # do an else here to find those that don't have any special
            # instructions
    ...
    def reprocessHTML(self, html):
        ...
        htmlContentMediaAdapted = self.handleImageVersions(
    ...
    def handleAudioVideoTags(self, htmlContent, mediaParams={}):
        ...
        while startIndex != -1:
            startIndex = self._findNextTagStart(
                htmlContentLower, startIndex, tagNames)
            if startIndex == -1:
                break
            ...
            # find the media base name without the extension and the current
            # extension
        ...
        # TODO: potential case sensitivity issue here -but exe always makes
        # lower case...
        ...
        if startScriptIndex != -1:
            ...
            htmlContent = htmlContent[:startScriptIndex] + \
                htmlContent[endScriptIndex:]
    ...
    def _getTagAttribVal(
        ...
        quoteUsed = ""
        if singleQuotePos != - \
                1 and (doubleQuotePos == -1 or singleQuotePos < doubleQuotePos):
            startAttribVal = singleQuotePos
            quoteUsed = "'"

        if doubleQuotePos != - \
                1 and (singleQuotePos == -1 or doubleQuotePos < singleQuotePos):
    ...
    def _findNextTagStart(self, htmlContent, startIndex, tagNames):
        ...
        for tagName in tagNames:
            ...
            if startThisTag != - \
                    1 and (startThisTag < lowestFound or lowestFound == -1):
