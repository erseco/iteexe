...
class EditorPane(object):
    ...
    def __init__(self, webServer, parent):
        ...
        self._purposeInstruc = x_(
            "The purpose dialogue allows you to describe"
            " your intended purpose of the iDevice to other"
            " potential users.")
        self._emphasisInstruc = x_(
            "Use this pulldown to select whether or not "
        ...
        self._textBoxInstruc = x_(
            "Add a text entry box to an iDevice. "
            "Used for entering larger amounts of textual "
            "content.")
        self._feedbackInstruc = x_(
            "Add an interactive feedback field to your iDevice.")
    ...
    def process(self, request, status):
        ...
        if "addTextArea" in request.args:
            field = TextAreaField(_("Enter the label here"), _(
...
data is entered into this field."""))
        ...
        if "addMP3" in request.args:
            ...
            field = MultimediaField(
                _("Enter the label here"),
                _("Enter the instructions for completion here"))
            field.setIDevice(self.idevice)
            self.idevice.addField(field)
            if 'xspf_player.swf' not in self.idevice.systemResources:
    ...
    def renderButtons(self, request):
        ...
        if self.idevice.edit:
            html += common.submitButton("preview",
                                        _("Preview"),
    ...
    def __renderStyles(self):
        ...
        # It's just a provisional solution so the user does not have to choose
        # the right Style
        html += "<script type='text/javascript'>\
