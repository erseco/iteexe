...
class SinglePageExport(object):
    ...
    def compruebaReproductores(self, node):
        ...
        for idevice in node.idevices:
            ...
            if not hasElpLink:
                hasElpLink = common.ideviceHasElpLink(
                    idevice, self.page.node.package)
            if not hasWikipedia:
        ...
        if hasElpLink or self.page.node.package.get_exportElp():
            ...
            currentPackagePath.copyfile(
                self.outputDir / self.page.node.package.name + '.elp')
        if hasWikipedia:
            wikipediaCSS = (self.cssDir / 'exe_wikipedia.css')
            wikipediaCSS.copyfile(self.outputDir / 'exe_wikipedia.css')
        if hasInstructions:
            common.copyFileIfNotInStyle(
                'panel-amusements.png', self, self.outputDir)
