...
class SortIdeviceInc(Idevice):
    ...
    def __init__(self, content=""):
        ...
        mainFieldInfo = {
            'title': [
                'text', x_('Title'), x_('Title to show')], 'instructions': [
                'textarea', x_('Instructions'), x_('Instructions before sortable list')], 'correctoverlay': [
                'textarea', x_('Correctly Sorted Overlay'), x_('Shown when check is clicked and correct')], 'wrongoverlay': [
                    'textarea', x_('Wrongly Sorted Overlay'), x_('Shown when check is clicked and wrong')], 'correcteffect': [
                        'choice', x_('Effect for showing correct overlay'), x_('Effect showing correct overlay'), {
                            'choices': EXEFIELD_JQUERYUI_EFFECTLIST}], 'wrongeffect': [
                                'choice', x_('Effect showing wrong answer overlay'), x_('Effect for showing wrong overlay'), {
                                    'choices': EXEFIELD_JQUERYUI_EFFECTLIST}], 'checkbuttontext': [
                                        'text', x_('Text for Check Button'), x_('Text for Check Button'), {
                                            "defaultval": x_("Check")}], 'checkbuttonstyle': [
                                                'text', x_('Check Button Style (CSS)'), x_('CSS for check button'), {
                                                    "defaultval": "color: white; background-color: green;", "type": "advanced"}], 'itemwidth': [
                                                        'text', x_('Width of an item in the list (px)'), x_('width'), {
                                                            "defaultval": "300", "type": "advanced"}], 'itemheight': [
                                                                'text', x_('Height of an item in the list (px)'), x_('height'), {
                                                                    "defaultval": "80", "type": "advanced"}], 'sortableitemstyle': [
                                                                        'text', x_('Style (CSS) for sortable items'), x_('Sortable Item CSS'), {
                                                                            "defaultval": "background-color: green; color: white; margin: 10px; padding: 5px;", "type": "advanced"}], 'sortorder': [
                                                                                'choice', x_('Sort Direction'), x_('Sort Direction'), {
                                                                                    'choices': [
                                                                                        [
                                                                                            "ttb", "Top To Bottom"], [
                                                                                                "ltr", "Left to Right"], [
        ...
        self.mainFieldSet = ExtendedFieldSet(
            self, mainFieldOrder, mainFieldInfo)
        self.mainFieldSet.makeFields()

        # array of TextAreaFields that are to be sorted (script will randomize
        # it)
        self.itemsToSort = []
        self.addItemToSort()
    ...
    def addItemToSort(self):
        newTextAreaField = TextAreaField(
            x_("Sortable Item"),
    ...
    def upgradeToVersion1(self):
        self.mainFieldSet.fieldInfoDict['sortorder'] = [
            'choice', x_('Sort Direction'), x_('Sort Direction'), {
                'choices': [
                    [
                        "ttb", x_("Top To Bottom")], [
                        "ltr", x_("Left to Right")], [
                        "rtl", x_("Right to Left")]]}]
        self.mainFieldSet.fieldOrder = [
            'instructions',
            'sortorder',
            'correctoverlay',
            'wrongoverlay',
    ...
    def upgradeToVersion3(self):
        self.mainFieldSet.fieldInfoDict['title'] = [
            'text', x_('Title'), x_('Title to show')]
