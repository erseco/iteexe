        ...
        for attribute in list(self._attributes.keys()):
            if hasattr(
                ...
                attribute.replace(
                    ...
                    '_')) != '':
                html += '<p><strong>' + \
                    _(self._attributes[attribute]) + ': </strong>'
                if attribute in self._attributesCode:
                    html += '<code>' + getattr(self,
                                               '_' + attribute.replace('-',
                                                                       '_')).replace('<',
                                                                                     '&lt') + '</code>' + '</p>'
                else:
            ...
            properties.append(
                {
                    ...
                    'value': value,
                    'format': self._attributes[attribute][1],
                    'nxml': attribute})
        return properties
        ...
        for attribute in list(self._attributes.keys()):
            if hasattr(
                ...
                attribute.replace(
                    '-',
                    '_')) != '':
                string += _(self._attributes[attribute]) + ': ' + \
                    getattr(self, '_' + attribute.replace('-', '_')) + '\n'
        return string
