...
langNames = {
    ...
    'ee': 'Eʋegbe',                     # Ewe
    # \xce\x95\xce\xbb\xce\xbb\xce\xb7\xce\xbd\xce\xb9\xce\xba\xce\xac, Greek
    'el': 'Ελληνικά',
    'en': 'English',
    ...
    'ru': 'Русский',                    # Russian
    # Sloven\xc4\x8dina, Slovensk\xc3\xbd Jazyk - Slovak
    'sk': 'Slovenčina, slovenský jazyk',
    'sl': 'Slovenščina',                # Sloven\xc5\xa1\xc4\x8dina - Slovene
    'sr': 'Српски / srpski',            # Srpski, serbio
    'sv': 'Svenska',                    # Swedish
    # \xe0\xb8\xa0\xe0\xb8\xb2\xe0\xb8\xa9\xe0\xb8\xb2\xe0\xb9\x84\xe0\xb8\x97\xe0\xb8\xa2 - Thai
    'th': 'ไทย',
...
class PreferencesPage(RenderableResource):
    ...
    def __init__(self, parent):
        ...
        for browser in mywebbrowser._browsers:
            if (browser not in browsersHidden):
                if browser in browserNames:
                    self.browsersAvalaibles.append(
                        (browserNames[browser], browser))
        self.browsersAvalaibles.sort()
        self.browsersAvalaibles.append(
            (_("Default browser in your system"), "None"))
        ...
        for licenses in common.getPackageLicenses():
            self.licensesNames.append(
                {'licenseName': licenses, 'text': _(licenses)})
    ...
    def render_GET(self, request):
        ...
        try:
            ...
            return json.dumps({'success': False,
                               'errorMessage': _("Failed to get preferences")})
        return json.dumps({'success': True,
                           'data': data,
                           'locales': self.localeNames,
                           'browsers': self.browsers,
                           'licensesNames': self.licensesNames})
    ...
    def render_POST(self, request):
        ...
        try:
            ...
            self.config.configParser.set(
                'user', 'internalAnchors', internalAnchors)
            ...
            self.config.configParser.set(
                'user', 'editorVersion', editorveresel)
            ...
            self.config.configParser.set(
                'user', 'showPreferencesOnStart', showPreferencesOnStart)
        except Exception as e:
            log.exception(e)
            return json.dumps({'success': False,
