...
class FileAttachBlockInc(Block):
    ...
    def renderEdit(self, style):
        ...
        html += "<br/>"
        html += common.submitButton("addFileAttachment" +
                                    str(self.id), _("Add Another File Attachment"))
        html += "<br/>"
    ...
    def _renderMain(self, style, previewMode=False):
        ...
        if showDesc == False and previewMode:
            html += "<p><strong> " + \
                _("File List (this list shows only in preview mode):") + "</strong></p>"
        ...
        for fileElement in self.fileAttachmentElements:
            if not showDesc:
                ...
                html += "<li><a href='%(prefix)s%(filename)s' target='_blank'>%(desc)s" % {
                    ...
                    "desc": fileElement.getDescription(),
                    "prefix": prefix}
