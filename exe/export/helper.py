...
def addListIdevicesFiles(node):
    ...
    listIdevicesFiles.append(
        common.printJavaScriptIdevicesScripts(
            'export', node))
    for child in node.children:
...
def exportMinFileJS(listFiles, outputDir):
    ...
    for inputFile in listFiles:
        ...
        # If the first line is empty, get the next one until one with content
        # pops up
        while line.strip(' ') == '\n':
            line = inputFileStream.readline()
...
def exportMinFileCSS(list_files, output_dir):
    ...
    for file in list_files:
        ...
        # This flag is used to prevent us from leaving the loop while reading a
        # multiline comment
        inside_comment = False
        for line in input_stream.readlines():
            # We only want to keep the comment if it's in the first written
            # line of the file
            ...
            # If the comment doesn't end here just write the line to the output
            # file
            if comment_end < 0:
                inside_comment = True
