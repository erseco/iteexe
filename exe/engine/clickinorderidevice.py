...
class ClickInOrderIdeviceInc(Idevice):
    ...
    def __init__(self, content=""):
        ...
        self.textAreaFieldNames = {
            'Instructions': [
                x_('Game Instructions'),
                x_('Instructions to show in leading paragraph')],
            'CompleteFeedback': [
                x_('Game Won Feedback Overlay'),
                x_('Shown over when the game is won')],
            'PositiveFeedback': [
                x_('Correct Answer Feedback Overlay'),
                x_('Shown over game when correct answer given')],
            'NegativeFeedback': [
                x_('Wrong Answer Feedback Overlay'),
                x_('Shown over game when wrong answer is given')],
            'MainArea': [
                x_('Main Area To Show'),
                x_('The Main Game Area on which to locate items')],
            'ClickToStartArea': [
                x_('Area shown before the game starts'),
                x_('User will click on this to start the game')]}
        self.textAreaFields = {}
        for textAreaFieldName, textAreaDesc in self.textAreaFieldNames.items():
            self.textAreaFields[textAreaFieldName] = TextAreaField(
                textAreaDesc[INDEX_TITLEFIELD], textAreaDesc[INDEX_INSTRUCTIONFIELD], "")
        ...
        self.textFieldNames = {
            'width': [
                x_('Game Width'),
                x_('Width of game area (in pixels)'),
                '300'],
            'height': [
                x_('Game Height'),
                x_('Height of game area (in pixels)'),
                '300'],
            'elementCounterStyle': [
                x_('Style of Element Counter'),
                x_('Use CSS to style element that counts number items found')],
            'hintWidth': [
                x_('Hint Width'),
                x_('Width (px) of hint area'),
                '300'],
            'hintHeight': [
                x_('Hint Height'),
                x_('Height (px) of hint area'),
                '50'],
            'hintAreaStyle': [
                x_('Hint Area Style'),
                x_('Style of Hint Area')],
            'timerStyle': [
                x_('Style of timer Element'),
                x_('Style of timer (CSS)')]}
        self.textFields = {}
        for textFieldName, textFieldDesc in self.textFieldNames.items():
            if len(textFieldDesc) >= INDEX_DEFAULTVALUEFIELD + 1:
                self.textFields[textFieldName] = TextField(
                    textFieldDesc[INDEX_TITLEFIELD],
                    textFieldDesc[INDEX_INSTRUCTIONFIELD],
                    textFieldDesc[INDEX_DEFAULTVALUEFIELD])
            else:
                self.textFields[textFieldName] = TextField(
                    textFieldDesc[INDEX_TITLEFIELD], textFieldDesc[INDEX_INSTRUCTIONFIELD])
        ...
        self.questionOrderChoiceField = ChoiceField(self, gameModeChoice, x_(
            "Shuffle Questions"), x_("Randomize question order or not"))
        self.timerChoiceField = ChoiceField(
            self, timerModeChoice, x_("Use Timer?"), x_("Use a Timer"))
    ...
    def addClickableField(self):
        newClickableArea = ClickInOrderClickableAreaField(
            x_("Clickable Area"), self)
...
class ClickInOrderClickableAreaField(Field):
    ...
    def __init__(
        ...
        self.textFieldNames = {
            'top': [
                x_('Top (Y) Coord'),
                x_('Top CSS top property')],
            'left': [
                x_('Left (X) Coord'),
                x_('Left CSS left property')],
            'width': [
                x_('Width (px)'),
                x_('Width of clickable area')],
            'height': [
                x_('Height (px)'),
                x_('Height of clickable area')],
            'hideDelay': [
                x_('Delay (ms) to hide revealed contents'),
                x_('After delay ms hide the content prev revealed')]}
        self.textFields = {}

        for textFieldName, textFieldDetails in self.textFieldNames.items():
            self.textFields[textFieldName] = TextField(
                textFieldDetails[INDEX_TITLEFIELD],
                textFieldDetails[INDEX_INSTRUCTIONFIELD])
            self.textFields[textFieldName].idevice = self.idevice

        self.textAreaFieldNames = {
            'Hint': [
                x_('Hint to Show for this item'),
                x_('The hint that will appear under the map for this item')],
            'ShowMe': [
                x_('Item Revealed'),
                x_('The Item that will be revealed in place')]}

        self.textAreaFields = {}

        for textAreaFieldName, textAreaFieldDetails in self.textAreaFieldNames.items():
            self.textAreaFields[textAreaFieldName] = TextAreaField(
                textAreaFieldDetails[INDEX_TITLEFIELD],
