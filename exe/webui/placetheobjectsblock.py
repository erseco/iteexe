...
class PlaceTheObjectsBlockInc(Block):
    ...
    def __init__(self, parent, idevice):
        ...
        self.positiveResponseElement = TextAreaElement(
            idevice.positiveResponseArea)
        self.negativeResponseElement = TextAreaElement(
            idevice.negativeResponseArea)
        self.clickToStartElement = TextAreaElement(
            idevice.clickToStartGameArea)
    ...
    def _makeObjectDivId(self, elementTypeName, placeableObjectElement):
        ...
        objectdivid = "placetheobjects" + self.id + "_" + \
            elementTypeName + placeableObjectElement.id
        return objectdivid
    ...
    def _renderForGame(self, style, previewMode=False):
        ...
        for placeableObjectElement in self.placableObjectElements:
            ...
            tolerance = int(
                placeableObjectElement.toleranceElement.renderView())
        ...
        for placeableObjectElement in self.placableObjectElements:
            ...
            elementDimensions = {
                "width": int(
                    placeableObjectElement.widthElement.renderView()), "height": int(
                    placeableObjectElement.heightElement.renderView())}
        ...
        html += "placeTheObjectsGameData['" + self.id + "']['showtimer'] = '" + str(
            self.gameTimerShown.field.content) + "';\n"
        html += "placeTheObjectsGameData['" + self.id + "']['timelimit'] = '" + str(
            self.gameTimeLimit.field.content) + "';\n"

        for placeableObjectElement in self.placableObjectElements:
            draggable_id = self._makeObjectDivId(
                "draggable", placeableObjectElement)
            droppable_id = self._makeObjectDivId(
                "droppable", placeableObjectElement)
            ...
            perItemCode = """
                ...
                "gameId": str(
                    self.id),
                "targetx": placeableObjectElement.targetXElement.renderView(),
                "targety": placeableObjectElement.targetYElement.renderView(),
                "width": placeableObjectElement.widthElement.renderView(),
                "height": placeableObjectElement.heightElement.renderView()}
            html += perItemCode
    ...
    def renderEdit(self, style):
        ...
        html += common.checkbox("enabletimer%s" % self.id,
                                self.idevice.gameTimerShown.content,
                                title=_("Enable Timer"),
                                instruction=_("Enable showing the timer in the game"))
        ...
        html += "<br/>"
        html += common.submitButton("addPlacableObject" +
                                    str(self.id), _("Add Placable Object"))
        html += "<br/>"
...
class PlacableObjectElement(Element):
    ...
    def __init__(self, field):
        ...
        # there is then field.maincontent, field.width, field.height,
        # field.correctx, field.correcty , etc.
    ...
    def process(self, request):
        ...
        field_engine_check_delete(
            self, request, self.field.idevice.objectsToPlace)
...
g_blockFactory.registerBlockType(
    PlaceTheObjectsBlockInc,
    PlaceTheObjectsIdeviceInc)
